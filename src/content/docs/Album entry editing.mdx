...
- The album has at least one name and the original language is specified. 
- All artists (including vocalists) responsible for the album are specified. Whenever possible, each of those artists should also have an artist entry. 
- The album has a cover picture, [temporary cover art tag](https://vocadb.net/T/429), or [no cover art tag](https://vocadb.net/T/2811/no-cover-art).
- The album includes one of the following: description, external link, or media link.
- The album's release date (at least the year) is specified.
- The album has a complete tracklist.
- The album type is specified.

##### Album entry validation errors

From [VocaDb.Model.Resources.json](https://github.com/VocaDB/vocadb/blob/60325c247287e2f0ca2e22a2604ac5a4c5e56898/VocaDbWeb/public/locales/quc/VocaDb.Model.Resources.json#L2):
...
- NeedCover, corresponds to [Rule: Cover art required](/docs/pinned/rule-reference#rule-cover-art-required).
- NeedReferences, corresponds to [Rule: Album source required](/docs/pinned/rule-reference#rule-album-source-required).
- NeedReleaseYear, corresponds to [Rule: Publish year required](/docs/pinned/rule-reference#rule-publish-year-required).
- NeedTracks, corresponds to [Rule: Rule: Tracklist status](/docs/pinned/rule-reference#rule-tracklist-status).
