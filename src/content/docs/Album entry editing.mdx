...
Support status is a special setting for a particular artist credit. The support status should be used when the album has a clear separation of main artists and support artists.

Usage examples (albums):

- Background illustrators (to distinguish them from the main illustrator)
- Guest artists.
  - For example, album [electroll by 8#Prince](http://vocadb.net/Al/3572) includes 5 guest remixers.
  - The same applies for vocalists: album named with "feat. Hatsune Miku" should have other vocalists marked as support. 

Support status removes the artist(s) from the artist string and affects the search through the ["Only main songs" -filter](https://vocadb.net/Search?searchType=Album&artistId[0]=175&artistParticipationStatus=OnlyMainAlbums).
...
TODO - which artists should be copied from song entries?
