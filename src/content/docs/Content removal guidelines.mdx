...
[Verified artists](/docs/artists/artist-verification) can:
- Protect their artist entry, preventing edits from regular users.
- [Remove some information without asking](/docs/other-guidelines/content-removal-guidelines#information-that-the-artist-can-freely-remove-without-asking) such as old aliases, images, non-music links, ...

Content removal guidelines do not apply to entries that are outside our [content policy](/docs/other-guidelines/content-policy).

## General guidelines

Content removal requests **must always be verified** in some way. Having a [verified artist account on VocaDB](/docs/artists/artist-verification) is one way of doing it. A post directly from an artist's official channel/other account **directed at VocaDB/VocaDB staff/mods** also satisfies this criteria.
...
Deleted entries stay visible if accessed through the direct entry URL, but are hidden from the search and other website functions. Individual entry revisions can also be hidden if they contain sensible information.
...
- Artist & album images can be deleted (or replaced) in all cases.

3) External links:
  
- In most cases, links should be **marked as "unavailable"** instead of deleting them.
...
If the animator participates in song entries as a "Producer" ("Music PV" or "Drama PV"), deletion criteria of Music Producer apply. Otherwise (= collaborating with someone else), same criteria as **illustrator**.
