...
SSL is used to secure the connection between VocaDB and the user, so that no one else can see the sent or received data. This is useful when authenticating (logging in or registering), though it covers everything on VocaDB.
