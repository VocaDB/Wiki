...
parent: Pinned
tags: ["draft", "editing", "guidelines", "moderation", "user", "vocadb"]
---
...
Reporting an entry also sends a notification to the editor. If the entry has **more than one editor**, notifications are **not sent**.

Reporting a specific **version** should be preferred for entries by multiple editors, since it **always sends a message** to the relevant editor. This can be done by selecting the specific version within the ["View Modifications"](https://wiki.vocadb.net/docs/songs/song-entry-page#view-modifications) page on any entry. 

### Report an error

This is the main method of submitting entry reports. It appears on all page types.

Albums and artists have four report types, while songs have five.

#### Broken PV (Song entries)
...
#### Invalid or missing information (all entries)
...
* missing information
    ...
    * A missing artist credit. 
    If possible, **always prefer adding the information yourself** instead of leaving a report.

#### Duplicate entry (all entries)
