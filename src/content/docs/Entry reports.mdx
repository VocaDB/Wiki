...
Invalid entry reports can be safely closed. Asking for proof and further clarifications under the reported entry is **recommended, but optional**. Reports for entries that are against the content policy, such as songs that are fully out-of-scope, can be immediately handled by deleting the relevant entry or entries, though it is still good practice to send a report so the editor is notified. 
