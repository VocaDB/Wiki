...
[Albums](https://wiki.vocadb.net/wiki/86/album-entry-editing#Release-event) and [songs](https://wiki.vocadb.net/wiki/89/song-entry-editing#Release-event) can be connected to a release event. This connection indicates that the album or song was released **during** or **for** that event. For physical events this generally means that the album was first distributed at that event, so the release event date matches album release date.

Events can also be [tagged](https://wiki.vocadb.net/wiki/28/tags) and event participants can be credited with [artist entries](https://wiki.vocadb.net/wiki/84/artist-entry-page).
