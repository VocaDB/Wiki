...
**Note:** If a song C is derived from song B, which in turn is derived from song A, you can link C to A directly if B has no song entry. The skipped version (B) should be mentioned in entry C.
...
- Always prefer adding individual people to songs over circles or groups if possible. Note that different sources may swap producer and group names.

- If the song has no known artist credits available, use the [entry missing artists tag](https://vocadb.net/T/2808/entry-missing-artists).
...
- The language(s) of the original lyrics are synced with the entry language.
...
**Always leave a source for the lyrics**. If transcribed/translated by yourself, mention it.
...
In some cases, use the entry description or comment section instead for visibility or to ask questions.
