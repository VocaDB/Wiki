...
The title section should only contain the name of the song. Vocalists, song types, producers, etc. should all be removed.

If the song has no name, use the [untitled tag](https://vocadb.net/T/6335/untitled) with an empty placeholder name.

For covers and other derived works, copying the aliases from the original entry is unnecessary.
...
Song entries can be roughly classified as "original" or "derived". "Original" entries are the "root nodes" of the [entry tree structure](https://en.wikipedia.org/wiki/Tree_structure).
...
- If the original song is out-of-scope, use the [original-out-of-scope](https://vocadb.net/T/3282/original-out-of-scope) -tag along with **a link to the original song** or **a reference in the song description**.
...
- If the out-of-scope original song has **5 or more derived** (remix or cover) song entries on VocaDB, it can be created as a [cover unifier entry](https://vocadb.net/T/6751/out-of-scope-cover-unifier). 

  - The original (out-of-scope) artists should be credited as "custom artists" on the cover unifier entry. If the entry has no related (in-scope) artist entries, it can be created with the [temporary artist entry](https://vocadb.net/Ar/139625). Note that out-of-scope cover unifier entries should not be created for songs that already have a UtaiteDB entry.
...
Secondary song types can be added as tags to the entry.
...
- The original producer should not be added to the song entry if the remix was produced by someone else. Instead, add the [arranger](https://vocadb.net/T/2947)/[remixer](https://vocadb.net/T/371) to the song entry.
...
- The original producer should not be added to the song entry if the cover was produced by someone else. Instead, add the [cover artist](https://vocadb.net/T/7368/cover-artist)/[voice manipulator](https://vocadb.net/T/485/voice-manipulator) to the song entry.
...
Currently, it's not possible to specify multiple songs as originals in the "original song" field. They should be mentioned in the song description and/or external links instead.
...
- The song has at least one name, and the original language can't be unspecified.
- The song has at least one artist in the "producer" role category.
- Derived song types need the original version specified in the original version field or the song description.
- Songs with no vocalists need the "instrumental" tag, unless the song type is "Instrumental" or "Drama PV".
- Song type isn't "Unspecified".

##### Song entry validation errors

From [VocaDb.Model.Resources.json](https://github.com/VocaDB/vocadb/blob/60325c247287e2f0ca2e22a2604ac5a4c5e56898/VocaDbWeb/public/locales/quc/VocaDb.Model.Resources.json#L64):

- DuplicateArtist (automatically fixed when the entry is saved), corresponds to [Rule: No duplicate information](/docs/pinned/rule-reference#rule-no-duplicate-information).
- NeedArtist, corresponds to [Rule: Artist credit required](/docs/pinned/rule-reference#rule-artist-credit-required).
- NeedOriginal, corresponds to [Rule: Specify original](/docs/pinned/rule-reference#rule-specify-original).
- NeedProducer, corresponds to [Rule: Producer entry credits required](/docs/pinned/rule-reference#rule-producer-entry-credits-required).
- NeedReferences, corresponds to [Rule: Song content policy: Song source required](/docs/pinned/rule-reference#rule-song-content-policy-song-source-required).
- NeedType, corresponds to [Rule: Correct entry subtype required](/docs/pinned/rule-reference#rule-correct-entry-subtype-required).
- NonInstrumentalSongNeedsVocalists, corresponds to [Rule: Vocalist entry credit required](/docs/pinned/rule-reference#rule-vocalist-entry-credit-required) & [Rule: Instrumental-tag](/docs/pinned/rule-reference#rule-instrumental-tag).
- RedundantEvent, corresponds to [Rule: No redundant events](/docs/pinned/rule-reference#rule-no-redundant-events)
- RedundantRoles (deprecated rule)
- UnspecifiedNames, corresponds to [Rule: Original language required](/docs/pinned/rule-reference#rule-original-language-required).
...
- For [finished song entries](/docs/other-guidelines/required-entry-fields#song-fields), artist credits in the "producer"-category (composers, arrangers, voice manipulators, etc.) **must be made with artist entries** (as opposed to crediting them as custom artists).

- Song entries with no known producers should include the artist entry [Unknown producer(s)](https://vocadb.net/Ar/23966).
...
All embeds should have the same version of the song (except for unrelated [intros](https://vocadb.net/T/6334/unrelated-intro) and [outros](https://vocadb.net/T/6494/unrelated-outro)). Do not add karaoke versions as embeds; instead, link them in the external links section or create a new entry if they are part of an album. Different PVs with different song lengths should be separate song entries. 
Remixes and other versions featuring musical changes should always be separate entries.
...
If you would like to make romanizations of lyrics for VocaDB, please read the [Romanization guidelines](/docs/other-guidelines/romanization-guidelines#what-if-i-want-to-romanize-song-lyrics-for-vocadb). For both translation and romanization, remember that pure machine-translations/romanizations are not allowed.
