...
`Original song`s ("オリジナル曲" in Japanese) are songs that have completely original content and aren't based on other songs. The `original song` is the version of the song that was published first. Later versions are considered derivatives **regardless of who made them**.
...
The `music PV` type is used for **remarkable** PVs of original songs, whether created by the original producer or someone else.

If the music PV has the **same length** as the original: 
- If the music PV is "official" AND if the original entry has no official upload (album track for example), **use the original entry**.
- Low-effort PVs (made with a single static image for example) can be added as "Other" media to the **original entry**. If the length does not match, it should not be added to the entry. New `Music PV` -entries **should not be created in either case for low-effort PVs**.
- In all the other cases, create a new `Music PV` -entry. Note that music PVs with only out-of-scope characters are allowed if the video has **at least 100k** views.
...
- For [finished song entries](/docs/other-guidelines/required-entry-fields#song-fields), artist credits in the "producer"-category (composers, arrangers, voice manipulators, ...) **must be made with artist entries** (as opposed to crediting them as custom artists).
