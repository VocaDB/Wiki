...
English translation can be added in a separate field below if the original description exists in a non-English language.
...
If the **song vocals are different**, either with a new voicebank or language, but the instrumental sounds like the original, then it's likely a **cover**. If the **song structure** or **genre** is different, then it's probably a **remix**. If both the instrumental and vocals are changed, we generally favor classifying the song as a remix.
...
Outside of Vocaloid world, a cover implies re-recording of the song. However, in the context of Vocaloid, the "cover" term is used when adding Vocaloid vocals to an existing instrumental recording.

- Cover type should be used if the same song is re-released with new lyrics (while also using the [changed lyrics tag](http://vocadb.net/T/2866/changed-lyrics)).

- Sometimes the original artist re-releases the song with new vocals. The cover song type should be used in this case, along with the [self-cover tag](http://vocadb.net/T/391/self-cover).
...
Other than the lack of vocals, an instrumental song should be the same as the original version. If the song is otherwise different, the remix or cover types should be preferred (with the instrumental tag).
...
In this tab, you can add or remove artist entries that have participated in the song.
...
Support status is a special setting for crediting artists that clearly play a minor role compared to the main artists. It should only be used when the song or album has a **clear separation** of main artists and support artists.

Usage examples:

- Backing vocals, for example, a background chorus when the main vocals are sung by another vocalist.
...
The artist's type (from their entry) determines their default roles. For example, artists of the "lyricist" type are marked as the lyricist role by default. When the artist role is different from the artist type, the correct artist role must be specified.

The number of available roles is limited, so these serve as **approximations**: The person responsible for the song image should be credited with the "illustrator"  role, even if it's technically something else (photographer for example).
...
Arrangers are the people handling instrumentation and producing the music. Most of the time this role is used for **remixes**, where the remixer is generally credited with the role, but the roles for composition and arrangement (music production) are occassionally divided for original songs as well.
...
The person who created the song. Generally, for original songs, the composer and arranger are the same person, but not always.
...
The person responsible for the image (including photographers, etc.) for either a PV or album cover. Booklet illustrations are also included under this role.

Corresponding artist type: [Illustrator](/docs/artists/artist-entry-editing#Illustrator).

#### Instrumentalist

A person who plays an instrument in the song/album, usually physical instruments like pianos or guitars.
...
The person responsible for [mastering](https://en.wikipedia.org/wiki/Mastering_(audio)).

Corresponding artist type: [Other individual](/docs/artists/artist-entry-editing#Other-individual).

#### Mixer

The person responsible for [mixing](https://en.wikipedia.org/wiki/Audio_mixing_(recorded_music)).
...
If a PV made for a song is remarkable or well-known in some way, it's encouraged to create a separate entry for that PV and link it to the original song.
...
A reprint is a PV that is copied from another service, and uploaded by a party unrelated to the original artist. Reprint implies that an original, freely available PV exists or existed.
...
For some songs, only "Other"-type PV may exist, without an original, if there is no authorized PV available. This is common for album-only songs.
