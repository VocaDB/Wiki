...
parent: UtaiteDB
tags: ["albums", "editing", "guidelines", "utaitedb"]
---
...
UtaiteDB allows the creation of separate entries for multiple versions of albums, such as limited and digital editions. When adding entries for these albums, make sure to include the edition type in parenthesis in the corresponding album title field. For example, //(初回限定盤)// or //(限定版)// would be included with the album name in the Non-English field (even if the album name itself is in English), while //(Limited Edition)// would be used in the Romanized and/or English field.
